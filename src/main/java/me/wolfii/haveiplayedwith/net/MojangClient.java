...
import me.wolfii.haveiplayedwith.store.MojangCache;
...
public final class MojangClient {
    ...
    private final ConcurrentHashMap<UUID, MojangCache> uuidMemory = new ConcurrentHashMap<>();
    ...
    public Optional<MojangCache> cached(UUID uuid) {
        MojangCache memory = uuidMemory.get(uuid);
        if (memory != null) {
            return Optional.of(memory);
        }
        Optional<MojangCache> stored = database.mojangCache(uuid);
        stored.ifPresent(cache -> uuidMemory.put(uuid, cache));
        return stored;
    }

    public boolean isStale(MojangCache cache) {
    ...
    public Optional<Profile> lookupName(String username) {
        ...
        Optional<MojangNameCache> stored = database.mojangNameCache(key);
        if (stored.isPresent()) {
    ...
    private void store(UUID uuid, String username) {
        MojangCache cache = new MojangCache(username, Instant.now());
